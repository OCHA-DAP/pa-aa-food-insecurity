...
def return_max_cs(date, admin2, df, status,adm0c,adm1c,adm2c):
    '''
    Return the maximum IPC value in an admin2 district.
    '''
    sub = df.loc[(df['date']==date)&(df[adm2c]==admin2)]
    mx = sub['area'].max()
    row = sub[['date', adm0c, adm1c, adm2c,status]].loc[sub['area']==mx]
    return row

def gen_csml1m2(ipc_path,bound_path,status,dates,adm0c="ADM0_EN",adm1c="ADM1_EN",adm2c="ADM2_EN"):
    ...
    for d in overlap['date'].unique():
        for a in overlap[adm2c].unique():
            row = return_max_cs(d, a, overlap,status,adm0c,adm1c,adm2c)
...
def main():
    ...
    for STATUS in STATUS_LIST:
        ...
        df.to_csv("Data/" + COUNTRY + '_admin2_fewsnet_' + df.date.min().strftime('%Y%m%d') +'_'+ df.date.max().strftime('%Y%m%d')+'_'+STATUS+ '.csv')
