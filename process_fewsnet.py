...
def merge_ipcstatus(inputdf_dict, adm0c, adm1c, adm2c):
    """
    Merge the three types of IPC projections (CS, ML1, ML2) to one dataframe
    Args:
        inputdf_dict: dict with df for each status (CS, ML1, ML2)
...
def main(country_iso3, config_file="config.yml"):
    ...
    for STATUS in STATUS_LIST:
        statusdf_dict[STATUS] = gen_csml1m2(
            PATH_FEWSNET,
            ADMIN2_PATH,
            STATUS,
    ...
    df_allipc = merge_ipcstatus(statusdf_dict, shp_adm0c, shp_adm1c, shp_adm2c)
